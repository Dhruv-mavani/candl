...
Market duration expires
    ...
    ├── No new buys allowed
    ├── Anyone can redeem any shareholder's position via force_redeem
...
There is a single redemption instruction, `force_redeem`, and it doesn't require the shareholder's own signature -- anyone can trigger it on a specific shareholder's behalf, paying only the network fee. A self-redemption is just the same call with the shareholder as both the position being redeemed and the one paying the fee. It cannot redirect funds or change the amount -- the payout destination and size are both derived from that shareholder's own on-chain position (`trader_position`), never from anything the caller supplies. This exists so a market never gets stuck forever just because one shareholder lost their wallet or never comes back: the creator (who wants their NFT back, see below) or anyone else can simply pay out every remaining holder. The frontend's "Redeem" button does exactly this -- one click redeems the caller's own shares plus every other remaining holder's.

### NFT Return

After settlement, the NFT is returned to the creator once every share has been redeemed via `force_redeem`. There is no grace period or timeout that returns the NFT early: as long as even one share is outstanding, the market stays in Settling. Because anyone can force-redeem any remaining holder, this never actually means "forever" in practice.
...
| Market settled | When all shares redeemed (via `force_redeem`) | Checked on interaction |
...
| **Settling** | Until all shares redeemed | force_redeem shares, claim royalties |
