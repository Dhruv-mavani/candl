...
**Decision**: `create_market` now transfers the rent-exempt minimum (`Rent::minimum_balance(0)`) from `creator` into `vault` as its very last step, keeping it independent of `real_sol_reserves` (buy/sell logic is unchanged -- it only ever adds/subtracts the curve-tracked amount, never dips into the seed). `force_redeem` refunds `vault`'s entire remaining balance to `creator` once `outstanding_shares` reaches 0 (full settlement) -- at that point `real_sol_reserves` is already 0, so the remaining balance is exactly the untouched seed. The frontend surfaces this as a one-time, refundable cost when a creator opens the "Create Market" dialog.
